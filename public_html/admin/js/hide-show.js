$( document ).ready(function() {
    ...
    $(".more-cities").on("click", function() {
        if($(this).text() == '+') {
            $($(this).parents()[2]).find(".cities").css("display", "block");
            $(this).html('-');
        } else if($(this).text() == '-') {
            $($(this).parents()[2]).find(".cities").css("display", "none");
            $(this).html('+');
        }
    });
    ...
    $(".more-pharmacies").on("click", function() {
        if($(this).text() == '+') {
            $($(this).parents()[2]).find(".pharmacies").css("display", "block");
            $(this).html('-');
        } else if($(this).text() == '-') {
            $($(this).parents()[2]).find(".pharmacies").css("display", "none");
            $(this).html('+');
        }
    });
